...
function branchAndBound(prob, #problem object
		...
		timeCap = 600, #maximum time in seconds
		warmStart = zeros(1), #warm start vector

		maxDepth = 100000, #maximum number of active nodes in the tree
		...
		branchDimSelectMethod = 3,
		UOE=5000, # buffer to add when resizing arrays containing nodes and bounds
		gap = 1e-10, # optimality gap for the algorithm
		eigGap = 1e-8, # convergence criterion for determining eigenavlues by power method
		eigCycles = 5000, # max number of iterations of the power method
	...
	function eigen_bound(y, oldub)
		...
		cutoff = oldub*(1-1e-6)
		for i=1:length(y)
			if y[i]==-1 #|| y[i]==1 #indicates that we are actually cycling through all columns in indices_variable?
	...
	betas=Float64[]
	if size(Q,1)>1
		lambdas, betas, =Arpack.eigs(Q, which=:LR, nev=1, tol=0.0, maxiter=3000)
	else
		lambdas=[1.0]
		betas=[1.0]
	end
