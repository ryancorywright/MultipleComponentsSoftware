...
for r=2:3
    ...
    k_split=repeat([k_each], r)
        ...
        useCuts=false
        useSOCs=false
        run_psd=@elapsed UB_psd, = getSDPUpperBound_gd_multiple_permutation(pitprops, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
        @show run_psd, UB_psd

        usePSDs=false
        useCuts=true
        useSOCs=true
        run_cuts=@elapsed UB_cuts, = getSDPUpperBound_gd_multiple_permutation(pitprops, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts, maxCuts=50)
        @show run_cuts, UB_cuts

        usePSDs=false
        useCuts=false
        useSOCs=true
        run_soc=@elapsed UB_soc, = getSDPUpperBound_gd_multiple_permutation(pitprops, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
        @show run_soc, UB_soc

        push!(results_run, ["Pitprops", n, r, sum(k_split), k_split,
            UB_psd, run_psd ,UB_cuts, run_cuts, UB_soc, run_soc
        ])
...
for r=2:3
    ...
    k_split=repeat([k_each], r)
        ...
        useCuts=false
        useSOCs=false
        run_psd=@elapsed UB_psd, = getSDPUpperBound_gd_multiple_permutation(normwine, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
        @show run_psd, UB_psd

        usePSDs=false
        useCuts=true
        useSOCs=true
        run_cuts=@elapsed UB_cuts, = getSDPUpperBound_gd_multiple_permutation(normwine, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts, maxCuts=50)
        @show run_cuts, UB_cuts

        usePSDs=false
        useCuts=false
        useSOCs=true
        run_soc=@elapsed UB_soc, = getSDPUpperBound_gd_multiple_permutation(normwine, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
        @show run_soc, UB_soc
...
for r=2:3
    ...
    k_split=repeat([k_each], r)
            ...
            useCuts=false
            useSOCs=false
            run_psd=@elapsed UB_psd, = getSDPUpperBound_gd_multiple_permutation(normionosphere, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
            @show run_psd, UB_psd

            usePSDs=false
            useCuts=true
            useSOCs=true
            run_cuts=@elapsed UB_cuts, = getSDPUpperBound_gd_multiple_permutation(normionosphere, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts, maxCuts=50)
            @show run_cuts, UB_cuts

            usePSDs=false
            useCuts=false
            useSOCs=true
            run_soc=@elapsed UB_soc, = getSDPUpperBound_gd_multiple_permutation(normionosphere, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
            @show run_soc, UB_soc
...
for r=2:3
    ...
    k_split=repeat([k_each], r)
            ...
            useCuts=false
            useSOCs=false
            run_psd=@elapsed UB_psd, = getSDPUpperBound_gd_multiple_permutation(normGeographic, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
            @show run_psd, UB_psd

            usePSDs=false
            useCuts=true
            useSOCs=true
            run_cuts=@elapsed UB_cuts, = getSDPUpperBound_gd_multiple_permutation(normGeographic, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts, maxCuts=50)
            @show run_cuts, UB_cuts

            usePSDs=false
            useCuts=false
            useSOCs=true
            run_soc=@elapsed UB_soc, = getSDPUpperBound_gd_multiple_permutation(normGeographic, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
            @show run_soc, UB_soc
...
for r=2:3
    ...
    k_split=repeat([k_each], r)
        ...
        useCuts=false
        useSOCs=false
        run_psd=@elapsed UB_psd, = getSDPUpperBound_gd_multiple_permutation(normcommunities, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
                # This may require a lot of memory to run succesfully
        @show run_psd, UB_psd

        usePSDs=false
        useCuts=true
        useSOCs=true
        run_cuts=@elapsed UB_cuts, = getSDPUpperBound_gd_multiple_permutation(normcommunities, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts, maxCuts=50)
        @show run_cuts, UB_cuts

        usePSDs=false
        useCuts=false
        useSOCs=true
        run_soc=@elapsed UB_soc, = getSDPUpperBound_gd_multiple_permutation(normcommunities, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
        @show run_soc, UB_soc
...
for r=2:3
    ...
    k_split=repeat([k_each], r)
        ...
        useCuts=false
        useSOCs=false
        run_psd=@elapsed -1.0#UB_psd, = getSDPUpperBound_gd_multiple_permutation(normarrhythmia, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
        UB_psd=-1.0 # This requires too much memory to be able to run reliably
        @show run_psd, UB_psd

        usePSDs=false
        useCuts=true
        useSOCs=true
        run_cuts=@elapsed UB_cuts, = getSDPUpperBound_gd_multiple_permutation(normarrhythmia, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts, maxCuts=50)
        @show run_cuts, UB_cuts

        usePSDs=false
        useCuts=false
        useSOCs=true
        run_soc=@elapsed UB_soc, = getSDPUpperBound_gd_multiple_permutation(normarrhythmia, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts)
        @show run_soc, UB_soc
...
for r=2:3
    ...
    k_split=repeat([k_each], r)
        ...
        useCuts=true
        useSOCs=true
        run_cuts=-1.0#@elapsed UB_cuts, = getSDPUpperBound_gd_multiple_permutation(normarrhythmia, r, k_split, usePSDs, useSOCs, useCuts , maxCuts=5)
        ...
        useCuts=false
        useSOCs=true
        run_soc=@elapsed UB_soc, = getSDPUpperBound_gd_multiple_permutation(micromass, r, k_split, usePSDs=usePSDs, useSOCs=useSOCs, useCuts=useCuts, highDim=true) # Needed, or won't run on 370 GB RAM
        @show run_soc, UB_soc
